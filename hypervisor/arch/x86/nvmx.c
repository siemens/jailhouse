...
int nvmx_cpu_init(struct per_cpu *cpu_data)
{
	...
	return page_map_create(hv_page_table, 0, PAGE_SIZE,
...
static bool nvmx_get_vmcs_address(struct per_cpu *cpu_data, u64 ptr,
				  u64 *address)
{
...
static bool nvmx_vmclear(struct registers *guest_regs,
			 struct per_cpu *cpu_data)
{
	...
	vmx_skip_emulated_instruction(X86_INST_LEN_VMCLEAR_RAX);
	return true;

failed:
	panic_printk("FATAL: VMCLEAR failed for %016lx\n", vmcs);
	return false;
}

static bool nvmx_vmptrld(struct registers *guest_regs,
			 struct per_cpu *cpu_data)
{
	u64 *vmcs_data = cpu_data->mapped_nested_vmcs->data;
	unsigned int n;
	...
	if (page_map_create(hv_page_table, vmcs, PAGE_SIZE,
			    (unsigned long)cpu_data->mapped_nested_vmcs,
			    PAGE_DEFAULT_FLAGS, PAGE_DEFAULT_FLAGS,
			    PAGE_DIR_LEVELS) < 0)
		goto failed;
	printk("current VMCS = %lx\n", vmcs);
	...
	vmx_skip_emulated_instruction(X86_INST_LEN_VMPTRLD_RAX);
	return true;

failed:
	panic_printk("FATAL: VMPTRLD failed for %016lx\n", vmcs);
	return false;
}

static bool nvmx_vmwrite(struct registers *guest_regs,
			 struct per_cpu *cpu_data)
{
	u32 instr_info = vmcs_read32(VMX_INSTRUCTION_INFO);
	unsigned long field = guest_regs->rdx;
	struct vmcs_field_info field_info;
	u64 value;
	...
	if (!(field_info.flags & FIELD_FLAG_VALID)) {
		panic_printk("FATAL: unsupported vmwrite to %08x\n", field);
		return false;
	}
	...
	if (!(field_info.flags & FIELD_FLAG_SHADOWED)) {
		...
		if (!vmcs_write64(field, value)) {
			panic_printk("FATAL: VMWRITE of %016lx at %08lx "
				     "failed\n", value, field);
			return false;
		}
		if (!vmcs_load(&cpu_data->vmcs))
	...
	vmx_skip_emulated_instruction(X86_INST_LEN_VMWRITE_REG_RDX);
	return true;
...
static bool nvmx_vmread(struct registers *guest_regs, struct per_cpu *cpu_data)
{
	u32 instr_info = vmcs_read32(VMX_INSTRUCTION_INFO);
	unsigned long field = guest_regs->rdx;
	struct vmcs_field_info field_info;
	...
	field_info = nvmx_vmcs_field_info(field);
	if (!(field_info.flags & FIELD_FLAG_VALID)) {
		panic_printk("FATAL: unsupported vmread from %08x\n", field);
		return false;
	}
	...
	guest_regs->rax = value;
	vmx_skip_emulated_instruction(X86_INST_LEN_VMREAD_RAX_RDX);
	return true;
...
static bool nvmx_run(struct registers *guest_regs, struct per_cpu *cpu_data,
		     bool launch)
{
...
static bool nvmx_exit(u32 reason, struct registers *guest_regs,
		      struct per_cpu *cpu_data)
{
	u64 *vmcs_data = cpu_data->mapped_nested_vmcs->data;
	unsigned int n;
...
enum nvmx_return nvmx_handle_exit(u32 reason, struct registers *guest_regs,
				  struct per_cpu *cpu_data)
{
	if (cpu_data->nested) {
		if (!nvmx_exit(reason, guest_regs, cpu_data))
			return NVMX_FAILED;
		goto vmsucceed;
	}
	...
	case EXIT_REASON_VMWRITE:
		if (!nvmx_vmwrite(guest_regs, cpu_data))
			return NVMX_FAILED;
		break;
	case EXIT_REASON_VMREAD:
		if (!nvmx_vmread(guest_regs, cpu_data))
			return NVMX_FAILED;
		break;
	case EXIT_REASON_VMLAUNCH:
	case EXIT_REASON_VMRESUME:
		if (!nvmx_run(guest_regs, cpu_data,
			      reason == EXIT_REASON_VMLAUNCH))
			return NVMX_FAILED;
		return NVMX_HANDLED;
	case EXIT_REASON_VMCLEAR:
		if (!nvmx_vmclear(guest_regs, cpu_data))
			return NVMX_FAILED;
		break;
	case EXIT_REASON_VMPTRLD:
		if (!nvmx_vmptrld(guest_regs, cpu_data))
			return NVMX_FAILED;
		break;
	case EXIT_REASON_VMXON:
		cpu_data->nested_vmcs_addr = -1;
		vmx_skip_emulated_instruction(X86_INST_LEN_VMXON_RAX);
		break;
	case EXIT_REASON_VMXOFF:
		cpu_data->nested_vmcs_addr = -1;
		vmx_skip_emulated_instruction(X86_INST_LEN_VMXOFF);
		break;
	case EXIT_REASON_INVEPT:
		vmx_skip_emulated_instruction(X86_INST_LEN_INVEPT);
		break;
